...
# TODO 这个函数需要清理不用的部分 包括cv 和 之前推荐tuwen什么的嵌入部分去除
def fit(model, loss_fn=None, Dataset=None, dataset=None, eval_dataset=None, valid_dataset=None, gen_dataset_fn=None,
        num_folds=None, dry_run=False, **kwargs):
  timer = gezi.Timer(reset=False)
  fit_fn = get_fit()
  ...
  for i, (input_, valid_input_) in enumerate(zip(inputs, valid_inputs)):
    ...
    if FLAGS.work_mode != 'train':
      train_name = None
    logging.debug('Round:', FLAGS.round, 'mode:', FLAGS.mode, \
    ...
    for fold in folds:
      ...
      info = fit_fn(model, loss_fn, Dataset, dataset, eval_dataset, valid_dataset, gen_dataset_fn=gen_dataset_fn, dry_run=dry_run, **kwargs)
  ...
  logger.info(f'ALL DONE! You may check log file: [tail {FLAGS.log_dir}/log.html*]')
  logger.info(f'Fit using {timer.elapsed_minutes():.2f} minutes, {timer.elapsed_hours():.2f} hours')
