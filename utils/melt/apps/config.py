...
flags.DEFINE_integer('batch_size', None, 'Batch size. default as im2text default')
flags.DEFINE_alias('bs', 'batch_size')
flags.DEFINE_integer('eval_batch_size', None, 'Batch size fore eval')
flags.DEFINE_alias('eval_bs', 'eval_batch_size')
...
flags.DEFINE_bool('drop_last', True, '')
...
flags.DEFINE_alias('grad_acc','gradient_accumulation_steps')
flags.DEFINE_bool('gradient_checkpointing', False, '')
flags.DEFINE_alias('gradc', 'gradient_checkpointing')

#-------flow
flags.DEFINE_float('num_epochs', None, '''Number of epochs to run trainer.
...
#-------model
# flags.DEFINE_string('model', '../working/base', '')
...
flags.DEFINE_bool('pretrain_restart', True, '')
flags.DEFINE_string('pretrain_epoch', None, '')
flags.DEFINE_string('pretrain_subdir', None, '')
...
flags.DEFINE_bool('model_name_args', False, '')
flags.DEFINE_alias('mna', 'model_name_args')
...
flags.DEFINE_bool('save_only', False, 'only start init and save model might be usefull to check model init vars')
flags.DEFINE_bool('save_graph', False, 'some model might not save any weights, and no error produce which is annoying, TODO FIXME, so just by default model.save_weights')
flags.DEFINE_alias('save_network', 'save_graph')
flags.DEFINE_bool('save_graph_slim', False, '')
flags.DEFINE_bool('save_weights', False, 'normaly do not need as we have saved model.h5 with graph and weights if fail will save model.h5 with weights only, so here True means model.h5 with graph and dum weights.h5 with weight only')
flags.DEFINE_bool('save_inter_models', False, 'by default False not save model.h5 only checkpoint if not is last')
flags.DEFINE_bool('load_graph', True, '')
flags.DEFINE_bool('load_weights_only', False, '')
flags.DEFINE_alias('lwo', 'load_weights_only')
flags.DEFINE_bool('load_by_name', True, '')
flags.DEFINE_bool('load_skip_mismatch', True, '')
flags.DEFINE_bool('restore_configs', False, '')
flags.DEFINE_alias('restore_alias', 'restore_configs')
flags.DEFINE_bool('save_optimizer', True, '')
flags.DEFINE_bool('save_scheduler', True, '')
flags.DEFINE_bool('saved_model', False, '')
flags.DEFINE_bool('save_checkpoint', False, '')
flags.DEFINE_bool('save_keras_model', True, '')
flags.DEFINE_bool('model2tb', False, '')
flags.DEFINE_float('save_interval_epochs', None, 'if 0 or None will not save(or set large number like 1e10), by default 1 epoch 1 model in modeldir/epoch, you can change to 2, 0.1 etc')
...
flags.DEFINE_bool('save_epoch_ckpt', False, '')
flags.DEFINE_bool('save_final', False, '')  
flags.DEFINE_integer('last_models', 0, '')
flags.DEFINE_integer('last_model', 0, '')
...
flags.DEFINE_bool('restore_from_latest', True, 'more safe to restore from recent but not latest')
flags.DEFINE_bool('restore_optimizer', True, '')
flags.DEFINE_bool('restore_scheduler', True, '')
flags.DEFINE_bool('del_inter_model', False, '')
flags.DEFINE_alias('del', 'del_inter_model')
flags.DEFINE_bool('del_inter_events', False, '')

#--------show
flags.DEFINE_integer('interval_steps', 20, '')
flags.DEFINE_alias('is', 'interval_steps')
flags.DEFINE_integer('valid_interval_steps', 20,   """for training suggest 100 if you want to see valid loss, and other valid metrics if set during training, 
                                                     you can check evaluate interval time and evaluate once time the ratio below 0.1,
                                                     However due to bug prone with tf new version, disable it by default to 0 (fixed tf 2.6)
                                                     """)                                                     
...
flags.DEFINE_bool('metric_eval', True, '')
...
flags.DEFINE_alias('do_eval', 'do_valid')
...
flags.DEFINE_bool('allow_valid_train', True, '')
...
flags.DEFINE_alias('fie', 'first_interval_epoch')
...
flags.DEFINE_bool('write_during_train', True, '')
flags.DEFINE_string('pred_key', 'pred', '')
flags.DEFINE_list('out_ignore_keys', [], '')
flags.DEFINE_list('show_keys', [], 'if only show specific keys results after eval')
...
flags.DEFINE_string('optimizer', None, 'follow squad of ukhst https://www.quora.com/Why-is-AdaDelta-not-favored-in-Deep-Learning-communities-while-AdaGrad-is-preferred-by-many-over-other-SGD-variants')
...
flags.DEFINE_alias('opt_eps', 'opt_epsilon')
...
flags.DEFINE_bool('opt_correct_bias', False, '')
flags.DEFINE_bool('opt_fused', True, '')
flags.DEFINE_bool('fused_layernorm', True, '')
flags.DEFINE_bool('opt_8bit', False, '')
flags.DEFINE_bool('find_unused_parameters', True, '')
flags.DEFINE_alias('fup', 'find_unused_parameters')
flags.DEFINE_string('scheduler', None, '')
flags.DEFINE_bool('set_to_none', False, '')
...
flags.DEFINE_float('learning_rate', None, """adam will be 0.001
...
flags.DEFINE_float('min_learning_rate', 1e-7, 'min learning rate used for dyanmic eval metric decay')
...
flags.DEFINE_float('loss_decay', 0.5, '')
flags.DEFINE_float('metric_decay', 0.5, '')
...
flags.DEFINE_bool('reset_optimizer', False, '')
flags.DEFINE_bool('reset_eval', True, 'for pretrain')
...
flags.DEFINE_bool('dynamic_learning_rate', False, '')
...
flags.DEFINE_float('num_epochs_per_decay', 0., 'follow nasnet')
...
flags.DEFINE_integer('freeze_epochs', 0, '')
...
flags.DEFINE_bool('use_finetune_step', False, '')
flags.DEFINE_bool('use_finetune2_step', False, '')

flags.DEFINE_float('clip_gradients', 1000, """follow im2text as 5.0 default, 
...
flags.DEFINE_bool('optimize_has_scope', True, '')

#----------train
flags.DEFINE_bool('train_only', False, '')
flags.DEFINE_bool('train_all', False, 'use for small dataset like competetion or standard dataset where use k folds for train/valid and use all k parts if set train_all==True')
flags.DEFINE_bool('train_scratch', False, '')
flags.DEFINE_alias('scratch', 'train_scratch')
flags.DEFINE_bool('train_allnew', False, '')
flags.DEFINE_alias('allnew', 'train_allnew')
flags.DEFINE_bool('clear_model', False, '')
flags.DEFINE_bool('clear', False, '')
...
flags.DEFINE_bool('disable_monitor', False, '')
flags.DEFINE_integer('log_level', 20, '20 is logging.INFO, 10 is logging.DEBUG')
flags.DEFINE_bool('no_log', False, '')
flags.DEFINE_bool('quiet', True, 'do not log per step info to screen but still in logfile')
flags.DEFINE_bool('verbose', None, '')
flags.DEFINE_integer('speed_level', 0, '')
flags.DEFINE_bool('sample', False, '')
flags.DEFINE_float('sample_frac', None, '')

#flags.DEFINE_string('mode', 'train', 'or predict')
flags.DEFINE_bool('freeze_graph_collections', True, '')
flags.DEFINE_bool('tf_dataset', False, '')
flags.DEFINE_alias('tfds', 'tf_dataset')

flags.DEFINE_bool('use_tower_loss', False, 'prefer to use horovod')
flags.DEFINE_bool('use_valid_loss', True, '')

flags.DEFINE_bool('recount_tfrecords', False, '')
flags.DEFINE_bool('recount_train_steps', False, '')
...
flags.DEFINE_integer('num_gpus', None, """How many GPUs to use. set 0 or 1 to disable multi gpu mode, set None by default to use all available gpus""")
flags.DEFINE_alias('gpus', 'num_gpus')
flags.DEFINE_bool('log_device_placement', False, """Whether to log device placement.""")
flags.DEFINE_bool('batch_size_per_gpu', False, '''  per gpu batch size should be dived by gpu num ?
...
#----------scope
flags.DEFINE_bool('add_global_scope', True, '''default will add global scope as algo name,
...
#----- cross fold
flags.DEFINE_bool('fold_name', False, '')
flags.DEFINE_integer('fold', None, '')
flags.DEFINE_integer('num_folds', None, 'if 5 means 5 fold cv')
flags.DEFINE_alias('folds', 'num_folds')
flags.DEFINE_bool('ensemble_metrics', False, '')
flags.DEFINE_alias('enm', 'ensemble_metrics')
flags.DEFINE_alias('emetrics', 'ensemble_metrics')
flags.DEFINE_alias('eme', 'ensemble_metrics')
flags.DEFINE_bool('ensemble_kaggle', False, '')
flags.DEFINE_alias('kaggle_ensemble', 'ensemble_kaggle')
flags.DEFINE_bool('folds_metrics', False, '')
flags.DEFINE_bool('folds_metrics_force', False, '')
flags.DEFINE_bool('log_all_folds', True, '')
...
#---------- input reader
flags.DEFINE_bool('feed_dataset', True, 'for non eager mode')
...
flags.DEFINE_bool('shrink_len', False, '')
flags.DEFINE_bool('bucket_sampler', False, '')
flags.DEFINE_bool('batch_sampler', False, '')

flags.DEFINE_bool('sparse_to_dense', True, '')
flags.DEFINE_bool('padded_tfrecord', False, '')
flags.DEFINE_integer('padding_val', 0, '')
flags.DEFINE_alias('padding_idx', 'padding_val')
flags.DEFINE_bool('hvd_sparse_as_dense', False, 'horovd see lays/otimipzer.py hvd.DistributedOptimizer init args')
...
flags.DEFINE_bool('cache_train', False, '')
flags.DEFINE_bool('cache_valid', False, '')
flags.DEFINE_bool('cache_valid_input', False, '')
flags.DEFINE_bool('cache_test', False, '')
flags.DEFINE_bool('cache_test_input', False, '')
flags.DEFINE_string('cache_file', '', '')
flags.DEFINE_string('eval_name', 'eval', '')
flags.DEFINE_bool('prepare', False, '--prepare means gen train.fea only')

# huggingface datasets
flags.DEFINE_bool('hug_caching', False, '')
flags.DEFINE_alias('caching', 'hug_caching')
flags.DEFINE_string('hug_cache_dir', None, '')
flags.DEFINE_bool('hug_pb', True, '')
flags.DEFINE_bool('hug_err', True, '')
flags.DEFINE_bool('hug_inputs', False, '')

flags.DEFINE_bool('full_validation_final', False, '')
...
flags.DEFINE_bool('run_valid_op', True, '')
flags.DEFINE_bool('show_eval', True, '')
flags.DEFINE_integer('max_metrics_show', 30, '')
flags.DEFINE_bool('eval_shuffle_files', False, '')
flags.DEFINE_bool('shuffle', True, 'shuffle means shuffle batch and train files')
flags.DEFINE_bool('shuffle_valid', False, 'shuffle means shuffle batch and valid files')
flags.DEFINE_bool('shuffle_batch', None, 'shuffle batch only')
flags.DEFINE_bool('shuffle_files', None, 'shuffle files only')
flags.DEFINE_bool('parallel_read_files', True, '')
flags.DEFINE_bool('fixed_random', False, '')
flags.DEFINE_integer('eval_seed', 2048, '')
flags.DEFINE_integer('seed', None, '')
flags.DEFINE_bool('fix_sequence', False, '')
...
flags.DEFINE_bool('adjust_global_step', False, '')
flags.DEFINE_integer('global_step', None, '')
flags.DEFINE_integer('global_epoch', None, '')

flags.DEFINE_bool('eager', False, 'fo tf1 setting to eager, as tf1 default is graph mode')
flags.DEFINE_bool('graph', False, 'for tf2 setting to graph, as tf2 default is eager mode, TODO now in melt.flow.flow.py using slim which not ok for tf2')
flags.DEFINE_bool('run_eagerly', False, 'for keras model.compile')
...
flags.DEFINE_bool('torch', False, 'torch_model Wether use torch model, if true and not torch_only, tf eager read, torch model')
flags.DEFINE_bool('torch_only', True, 'torch_read Wether use torch reader, if true, torch read, torch model')
flags.DEFINE_alias('to', 'torch_only')
flags.DEFINE_bool('torch_lr', True, 'False might not work as tf learning rate problem')
flags.DEFINE_bool('torch_restart', False, '')
flags.DEFINE_bool('tf_only', False, '')
flags.DEFINE_alias('tfo', 'tf_only')

flags.DEFINE_bool("keras", None, "using keras fit")
flags.DEFINE_bool('keras_validation', False, '')
...
flags.DEFINE_bool('keras_loop', False, '')
flags.DEFINE_bool('predict_on_batch', False, '')
flags.DEFINE_bool('cache_info_dataset', False, '')
flags.DEFINE_integer('print_depth', 1, '')
flags.DEFINE_bool('keras_functional_model', False, '')
flags.DEFINE_alias('functional_model', 'keras_functional_model')
flags.DEFINE_alias('kfm', 'keras_functional_model')
flags.DEFINE_bool('keras_custom_progress_bar', True, '')
...
flags.DEFINE_bool('rich_tqdm', False, '')
flags.DEFINE_alias('rtqdm', 'rich_tqdm')
flags.DEFINE_string('tqdm_colour', None, 'None as white, cyan, blue')
flags.DEFINE_integer('tqdm_nrows', None, '')
...
flags.DEFINE_bool('eval_leave', False, '')
...
flags.DEFINE_bool('print_metrics', True, '')
flags.DEFINE_bool('print_lr', False, '')

flags.DEFINE_bool('test_aug', False, '')

flags.DEFINE_integer('batch_size_dim', 0, '')

flags.DEFINE_bool('simple_parse', False, '')
...
flags.DEFINE_bool('enable_xla', False, '')
flags.DEFINE_string(
    "tpu_name", None,
...
flags.DEFINE_bool('horovod_eval', True, 'wether using multiple gpu for eval and infer, hvd.allgather not work for tf ... currently, mpi4py ok')
flags.DEFINE_bool('horovod_shard', True, 'only consider train, valid/test always shard, if shard then each rank got 1/n files')
flags.DEFINE_bool('horovod_fp16', False, 'wether compression use fp16 see layers/optimizer.py')
flags.DEFINE_bool('distributed_scale', None, '') 
flags.DEFINE_bool('shard_by_files', True, '如果不能文件均分设置False或者使用自定--use_shard=False')
...
flags.DEFINE_bool('debug_lr', False, '')
flags.DEFINE_bool('debug_model', False, '')
flags.DEFINE_bool('model_summary', False, '')
flags.DEFINE_alias('summary_model', 'model_summary')

flags.DEFINE_integer('profile_interval_steps', 0, '')
flags.DEFINE_bool('profiling', False, 'alias for enable_profiling')
flags.DEFINE_bool('enable_profiling', False, 'for torch now')
flags.DEFINE_bool('valid_perf', False, '')

flags.DEFINE_integer('wait_gpu_span', 10, '0 means not wait')

flags.DEFINE_bool('monitor_gradients', False, '')
flags.DEFINE_bool('monitor_global_gradients', False, '')
flags.DEFINE_bool("monitor_l2", True, "")

flags.DEFINE_bool("write_summary", False, "")
flags.DEFINE_bool("write_metric_summary", False, "")
flags.DEFINE_alias('write_metrics_summary', 'write_metric_summary')
flags.DEFINE_bool("train_valid_summary", False, "")
...
flags.DEFINE_bool("tf_v2", False, "")

flags.DEFINE_bool("allow_soft_placement", True, "")
flags.DEFINE_bool("allow_growth", True, "")
flags.DEFINE_bool("simple_startup", False, "")

# hack for CloudS
flags.DEFINE_bool('sync_hdfs', True, '')
flags.DEFINE_string('hdfs_mark', 'publicData', '')
flags.DEFINE_string('local_mark', 'localData', '')

#--------------- other
flags.DEFINE_string('run_version', None, '')
flags.DEFINE_alias('rver', 'run_version')
flags.DEFINE_bool('is_online', False, '')
flags.DEFINE_alias('online', 'is_online')
flags.DEFINE_alias('ol', 'is_online')
...
flags.DEFINE_bool('hack_infer', False, '')
flags.DEFINE_float('hack_rate', 0.1, '')
flags.DEFINE_bool('simulate_test', False, '')

flags.DEFINE_bool('use_pseudo', False, '')
...
flags.DEFINE_bool('async_eval', True, 'only cpu evaluation without prediction(already done and write to csv)')
flags.DEFINE_bool('async_eval_last', False, '')
...
flags.DEFINE_bool('sync_valid_hour', False, '')
flags.DEFINE_bool('sync_valid_hour_final', False, '')
...
flags.DEFINE_bool('exit_after_eval', False, '')
flags.DEFINE_bool('exit_after_epoch', False, '')
...
flags.DEFINE_bool('amp_infer', True, '')
flags.DEFINE_bool('fp16_infer', False, '')
...
flags.DEFINE_bool('out_hook_per_batch', True, '')
...
flags.DEFINE_integer('workers', 20, '')
flags.DEFINE_integer('num_workers', 16, 'mainly for torch dataset')
flags.DEFINE_bool('pin_memory', False, '')
flags.DEFINE_alias('pin_mem', 'pin_memory')
...
flags.DEFINE_integer('wandb_factor', 5, 'valid_inteval_steps * wandb_factor to log wandb avoid too many')
flags.DEFINE_bool('wandb_scratch', False, 'wandb from scratch')
flags.DEFINE_alias('ws', 'wandb_scratch')
flags.DEFINE_bool('wandb_overwrite', False, '')
flags.DEFINE_alias('wo', 'wandb_overwrite')

# kaggle
flags.DEFINE_bool('kaggle_suffix', False, '')
flags.DEFINE_integer('kaggle_idx', None, '')
flags.DEFINE_alias('kidx', 'kaggle_idx')
flags.DEFINE_string('kaggle_name', None, '')
flags.DEFINE_string('kaggle_method', 'update', 'kaggle dataset method update or create')
flags.DEFINE_alias('kmethod', 'kaggle_method')
flags.DEFINE_bool('kaggle_all', False, '')
flags.DEFINE_bool('kaggle_online', False, '')
flags.DEFINE_integer('kaggle_folds', 5, '')
flags.DEFINE_alias('kfolds', 'kaggle_folds')
flags.DEFINE_alias('kall', 'kaggle_all')
flags.DEFINE_integer('kaggle_fold', None, '')
flags.DEFINE_bool('kaggle_prepare', False, '')
flags.DEFINE_alias('kaggle_upload', 'kaggle_prepare')
flags.DEFINE_alias('kprepare', 'kaggle_prepare')
flags.DEFINE_alias('kupload', 'kaggle_prepare')
...
flags.DEFINE_float('rdrop_loss_rate', 0., '')
flags.DEFINE_alias('rdrop_rate', 'rdrop_loss_rate')
flags.DEFINE_float('rdrop_start_epoch', 0., '')
flags.DEFINE_float('rdrop_end_epoch', 1e10, '')

# 对抗训练等训练策略
flags.DEFINE_integer('adv_epochs', None, '')
flags.DEFINE_bool('awp_train', None, '') 
flags.DEFINE_bool('fgm_train', None, '') 
flags.DEFINE_bool('ema_train', None, '')
flags.DEFINE_float('ema_start_epoch', 1, '')
flags.DEFINE_float('ema_decay', 0.999, '')
flags.DEFINE_float('adv_start_epoch', 1, '') 
flags.DEFINE_float('adv_eps', None, '')
flags.DEFINE_float('adv_lr', 1., '')