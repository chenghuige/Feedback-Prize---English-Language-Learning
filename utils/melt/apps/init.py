...
from gezi import logging, logger, logger2
...
def reset(reset_all=True):
  ...
  FLAGS.reset_all = reset_all
  FLAGS.batch_size = gezi.get('ori_batch_size')
  FLAGS.eval_batch_size = gezi.get('ori_eval_batch_size')
  FLAGS.lr = gezi.get('ori_lr')
  FLAGS.learning_rates = gezi.get('ori_lrs')

def reinit(reset_all=True):
  reset(reset_all=reset_all)
  init()
...
def _restart(allow_cpu=False):
  ...
  if ((len(gpus) < num_gpus) or FLAGS.cpu_valid or
      gezi.get_env('CUDA_VISIBLE_DEVICES') == '-1') and (allow_cpu and
                                                         FLAGS.allow_cpu):
    logger.info('restart as CUDA_VISIBLE_DEVICES=-1')
    ...
    logger.info(f'restart as CUDA_VISIBLE_DEVICES={gpus_str}')
    command = f'CUDA_VISIBLE_DEVICES={gpus_str} {command}'
  if not command.endswith('&'):
    command += ' &'
  gezi.system2(command)
  exit(0)
...
def init(graph=None):
  ...
  if FLAGS.forever:
    FLAGS.num_epochs = 1e20
    FLAGS.save_interval_epochs = 1e20
    FLAGS.wandb = False
    FLAGS.train_only = True
    FLAGS.write_summary = False
    FLAGS.write_metric_summary = False
  
  from icecream import ic
  
  import datasets
  try:
    datasets.utils.set_progress_bar_enabled(FLAGS.hug_pb)
    if FLAGS.hug_err:
      datasets.utils.logging.set_verbosity_error()
    datasets.set_caching_enabled(FLAGS.hug_caching)
    datasets.config.MAX_TABLE_NBYTES_FOR_PICKLING = 1 << 30
    logging.debug(FLAGS.hug_caching, FLAGS.hug_err, FLAGS.hug_pb)
  except Exception:
    pass
  ...
  if FLAGS.folds_metrics_force:
    FLAGS.folds_metrics = True
  ...
  if FLAGS.seed:
    gezi.seed_everything(FLAGS.seed)

  np.set_printoptions(precision=FLAGS.print_precision)
  
  gezi.set('ori_batch_size', FLAGS.batch_size)
  gezi.set('ori_eval_batch_size', FLAGS.eval_batch_size)
  gezi.set('ori_lr', FLAGS.lr)
  gezi.set('ori_lrs', FLAGS.learning_rates)
  ...
  if FLAGS.model_name:
    if FLAGS.kaggle_suffix and os.path.exists(
  ...
  if FLAGS.model_name_suffix and FLAGS.mode != 'async_valid':
    if not any(FLAGS.model_name_suffix.startswith(x) for x in ['.', '_', '-']):
       FLAGS.model_name_suffix = f'.{FLAGS.model_name_suffix}'
  ...
  if not FLAGS.model_name and FLAGS.model_dir:
    ...
    try:
      x = int(FLAGS.model_name)
      FLAGS.model_name = os.path.basename(os.path.dirname(FLAGS.model_dir))
    except Exception:
      pass
  ...
  if (not FLAGS.clear_first):
    assert not (FLAGS.check_exists and model_exists and mode == 'train')
    if (FLAGS.train_scratch or 'SCRATCH' in os.environ) and (FLAGS.work_mode == 'train') and (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.steps < 0) and (not FLAGS.folds_metrics):
      if os.path.exists(os.path.join(FLAGS.model_dir, 'log.html')):
        logs = open(f'{FLAGS.model_dir}/log.txt').readlines()
        if logs:
          logging.info('In scratch mode and found model_dir',
                        FLAGS.model_dir, 'exit(0)')
          exit(0)
      # if os.path.exists(FLAGS.model_dir):
      #   logger.info('In scratch mode and found model_dir',
      #                   FLAGS.model_dir, 'exit(0)')
        # exit(0)
    if model_exists and FLAGS.train_allnew and (FLAGS.work_mode == 'train') and (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.folds_metrics) and (FLAGS.kaggle_idx is None):
      ic(FLAGS.model_dir, model_exists, FLAGS.train_all, FLAGS.train_allnew, 'ignore and exist')
      exit(0)
      
    if (FLAGS.mode == 'train' or FLAGS.mode is None) and os.path.exists(f'{FLAGS.model_dir}/done.txt') and (not FLAGS.folds_metrics) and (not FLAGS.kaggle_method):
      ic('Find done.txt')
      exit(0)

    if (not FLAGS.allow_loop) and model_exists and os.path.exists(f'{FLAGS.model_dir}/metrics.csv') \
      and  (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.steps < 0) \
        and (not FLAGS.folds_metrics) and (not FLAGS.kaggle_method):
      num_valid_steps = FLAGS.nvs if FLAGS.nvs else int(FLAGS.epochs / FLAGS.vie)
      df = pd.read_csv(f'{FLAGS.model_dir}/metrics.csv')
      if len(df) == num_valid_steps:
        ic('Has been done with metrics', df)
        ic(FLAGS.model_dir)
        exit(0)
  ...
  gezi.system2(f'mkdir -p {FLAGS.ori_log_dir}')
  ...
  gezi.system2(f'mkdir -p {FLAGS.ori_model_dir}')
  ...
  if dist.local_rank() == 0:
    if FLAGS.clear_first and FLAGS.model_dir and os.path.exists(FLAGS.model_dir):
      ...
      wandb_id = gezi.read_str_from(f'{FLAGS.model_dir}/wandb_id.txt')
      os.system('rm -rf %s/*' % FLAGS.model_dir)
      if wandb_id:
        gezi.write_to_txt(wandb_id, f'{FLAGS.model_dir}/wandb_id.txt')
  ...
  if FLAGS.gcs_src:
    gezi.system2(f'gsutil -m cp -r {FLAGS.gcs_src}/* {FLAGS.model_dir}')

  if FLAGS.restore_configs:
    pretrained = None
    if FLAGS.pretrained:
      pretrained = f'{os.path.dirname(FLAGS.model_dir)}/{FLAGS.pretrained}' if not os.path.exists(FLAGS.pretrained) else FLAGS.pretrained
      assert os.path.exists(pretrained), pretrained
    # TODO flags alias是一个风险 比如--pt如果是pretrain的alias一定要在下面ignore写上pt 否则还是等于restore了 FLAGS.pretrain
    ignore_names = [
            'restore_configs', 'retore_flags',
            'mode', 'work_mode', 'num_replicas', 'num_gpus', 'gpus', 'log_dir', 'model_dir',
            'model_name', 'mname', 'mn', 'pretrained_dir', 'pretrained', 'pretrain',
            'load_weights_only', 'load_by_name', 'load_skip_mismatchs',
            'batch_size', 'bs', 'eval_batch_size', 'eval_bs', 'fp16', 'fold', 'folds', 'input',
            'train_input', 'valid_input', 'test_input', 'train_exclude',
            'valid_exclude', 'train_only', 'max_metrics_show',
            'train_files', 'input_files', 'inputs', 
            'valid_files', 'valid_inputs', 'test_files', 'distributed',
            'workers', 'num_workers', 'kaggle_idx', 'kidx', 'kaggle_method', 'kmethod',
            'wandb', 'wd', 'wandb_scratch', 'ws', 'wandb_group', 'wg', 
            'wandb_id', 'wandb_project', 'wandb_name', 'wandb_entity', 'we', 'wandb_overwrite', 'wo',
        ] + gezi.get_commandline_flags()
    gezi.restore_configs(
        pretrained or FLAGS.log_dir or FLAGS.model_dir,
        ignores=ignore_names)
  ...
  logtostderr = True if FLAGS.mode != 'async_valid' else False
  if dist.local_rank() != 0:
    FLAGS.quiet = True
    logtofile = False
    logtostderr = False
  ...
  if FLAGS.write_summary:
    FLAGS.write_metrics_summary = True
  ...
  #     logger.error('torch mode must set CUDA_VISIBLE_DEVICES restart')
  #     _restart(allow_cpu=False)
  if FLAGS.num_gpus is None:
    if num_specific_gpus:
      FLAGS.num_gpus = num_specific_gpus
  ...
  if FLAGS.num_workers is None:
    FLAGS.num_workers = max(FLAGS.num_gpus, 1) * 4
  ...
  def _exit_fn(num_free_gpus):
    if FLAGS.mode == 'async_valid':
      ...
      logger.warning('Train:Timeout waiting gpu exit -1')
      exit(-1)
  ...
  if distributed:
    ...
    logging.debug('distributed', distributed, dist.dist)
    ...
    logging.debug('In distributed mode using %d gpus' % dist.size())
    ...
    if FLAGS.distributed_scale:
      ...
      logging.debug('using distributed multipy learning rate by {} to {}'.format(
  ...
  if FLAGS.buckets:
    ...
    if FLAGS.batch_parse:
      logger.warning('Buckets dataset force to set batch_parse to False')
  ...
  # using tf.dataset to read tf records and for torch will convert once 
  # FLAGS.torch_only will assume if FLAGS.torch mode get dataset from torch tensor directly no need of cast from tf tensors
  if FLAGS.tf_dataset:
  ...
  if FLAGS.torch:
    ...
    # tf dataset 模式 repeat 所以无所谓 所有训练batch 还是满batch
    FLAGS.drop_remainder = False
    
  if FLAGS.torch and (not FLAGS.tf_dataset):
    # torch dataset 模式训练是按照默认drop last的 包括eager/train.py也根据drop last处理
    ...
    if FLAGS.torch:
      if tf.__version__ < '2':
        ...
        if not gezi.get('tpu'):
          ...
          if physical_devices:
            tf.config.set_visible_devices([physical_devices[x] for x in gpus], 'GPU')
  ...
  if not FLAGS.torch:
    ...
    if not FLAGS.torch_only:
      ...
      logging.debug('Torch dataset and Torch model train, distributed:',
                   distributed)
  ...
  if 'NUM_EPOCHS' in os.environ:
    ...
    logger.info('Using num_epochs set from env as %d' % FLAGS.num_epochs)
  ...
  if 'NUM_STEPS' in os.environ:
    ...
    logger.info('Using num_steps set from env as %d' % FLAGS.num_steps)
  ...
  if 'LEARNING_RATE_DECAY_FACTOR' in os.environ:
    ...
    logger.info('Using learning_rate_decay_factor set from env as %d' %
                 FLAGS.learning_rate_decay_factor)

  if 'BUCKETS' in os.environ:
    FLAGS.buckets = os.environ['BUCKETS']
    logger.info('Using buckets set from env as %d' % FLAGS.buckets)

  if 'BATCH_SIZES' in os.environ:
    FLAGS.batch_sizes = os.environ['BATCH_SIZES']
    logger.info('Using batch sizes set from env as %d' % FLAGS.batch_sizes)

  if 'NUM_LAYERS' in os.environ:
    FLAGS.num_layers = int(os.environ['NUM_LAYERS'])
    logger.info('Using num layers set from env as %d' % FLAGS.num_layers)
  ...
  if not FLAGS.distributed:
    ...
    FLAGS.batch_size = int(FLAGS.batch_size / dist.size())
    FLAGS.eval_batch_size = int(FLAGS.eval_batch_size / dist.size())
  ...
  ic(global_batch_size, acc_steps, replica_batch_size, eval_batch_size, num_gpus, gpus, CUDA_VISIABLE_DEVICES, work_mode, distributed, seed)
    ...
    if FLAGS.work_mode == 'train':
      ...
      while True:
        ...
        if not dirs_ or melt.get_num_records_from_dir(
          ...
          count += 1
          if count == 10:
            logger.error('could not find valid hour tfrecord')
            exit(0)
        if dirs_:
          dirs = dirs_
          break
  ...
  ic(FLAGS.model_name, FLAGS.model_dir)
  if FLAGS.model_dir != FLAGS.log_dir:
    logging.debug(f'model: [{FLAGS.model_name}]', 'model_dir:',
  ...
  try:
    # TODO NOTICE v100 fp16 is ok, bug on a100 too slow.. so only open for v100 now
  ...
  logging.debug('Train precision:', gezi.get('precision_policy_name'))
  ic(FLAGS.fp16, gezi.get('precision_policy_name'))
  ...
  if FLAGS.reset_optimizer:
    ...
    gezi.set('optimzers', None)

  if FLAGS.num_epochs > 10000 or FLAGS.num_epochs <=0:
    FLAGS.wandb = False
    FLAGS.write_summary = False
  ...
  if not gezi.get('wandb_run'):
    try:
      ...
      if FLAGS.wandb:
        ...
        if wandb_id:
          if not FLAGS.wandb_scratch:
            if FLAGS.wandb_resume is None:
        ...
        FLAGS.wandb_name = FLAGS.wandb_name or FLAGS.model_name
        ic(FLAGS.wandb_group, FLAGS.wandb_dir, FLAGS.wandb_project, FLAGS.wandb_name)
        run = wandb.init(project=FLAGS.wandb_project,
                        # job_type='Offline' if not FLAGS.online else 'Online',
        ...
        # else:
        #   ic(group_folds)
        #   FLAGS.wandb_group = group_folds
        #   FLAGS.wandb_dir = wandb_dir_folds
        #   FLAGS.wandb_name = FLAGS.wandb_name or FLAGS.model_name
        #   ic(FLAGS.wandb_group, FLAGS.wandb_dir, FLAGS.wandb_project, FLAGS.wandb_name)
        #   run = wandb.init(project=FLAGS.wandb_project,
        #           group=group_folds,
        ...
        #           sync_tensorboard=FLAGS.wandb_tb,
        #           magic=FLAGS.wandb_magic)
        logging.debug('wand_url:', run.url)
        ic(run.url)
        ...
        if not run:
          logger.warning('Turn off wandb due to error like fail to connect')
          FLAGS.wandb = False
  ...
  if FLAGS.local_rank == 0:
    ...
    commands_file = '../working/commands'
    ...
    if FLAGS.mode != 'async_valid':
      ...
      gezi.append_to_txt(FLAGS.command, commands_file)
  ...
  if FLAGS.local_rank == 0:
    if FLAGS.folds_metrics:
      gezi.folds_metrics_wandb(FLAGS.model_dir, FLAGS.folds, FLAGS.folds_metrics_force)
