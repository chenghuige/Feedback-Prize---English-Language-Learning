...
def line(dfs, y=[], x='step', color=None, names=None, smoothing=0., line_smoothing=0., mean_smooth=False, include=None, exclude=None, 
    ...
    try:
      ...
      for df in dfs:
        ...
        try:
          data = go.Scatter(
            x=xs,
            y=gezi.smooth(ys, smoothing) if not mean_smooth else gezi.mean_smooth(ys),
            mode=mode,
            line_shape='spline',
...
def text_heatmap(tokens, probs, min_prob=0.):
  import spacy
  import matplotlib 
  import matplotlib.pyplot as plt
  ...
  colors = [matplotlib.colors.to_hex(cmap(x)[:3]) for x in probs]
  text = ' '.join(tokens)
  start, end = 0, 0
  ents = []
  ...
  colors = {str(i): colors[i] for i in range(len(colors))}
  options = {"colors": colors, "ents": list(colors.keys())}
  spacy.displacy.render(doc, style='ent', options=options , manual=True, jupyter=True)
  
def his_counts(df, field, nbins=None):
  return px.histogram(df.value_counts(field).reset_index(name='count'), field, 'count', nbins=nbins)

def bar_counts(df, field, orientation='h', ascending=False, inplace=False):
  if inplace:
    df_ = df
  else:
    df_ = df.copy()
    df_[field] = df[field].astype(str)
    df_ = df_.value_counts(field).reset_index(name='count')
  if ascending is not None:
    df_ = df_.sort_values('count', ascending=ascending)
  if orientation == 'h':
    return  px.bar(df_, 'count', field, orientation='h')
  return px.bar(df_, field, 'count')

def pie_counts(df, field, inplace=False):
  if inplace:
    df_ = df
  else:
    df_ = df.copy()
    df_[field] = df[field].astype(str)
  return px.pie(df_.value_counts(field).reset_index(name='count'), field, 'count')

def bar_sorted(ys, xs, topn=0):
  ys = np.asarray(ys)
  xs = np.asarray(xs)
  sorted_idxes = np.argsort(ys)
  if topn > 0:
    sorted_idxes = sorted_idxes[-topn:]
  else:
    sorted_idxes = sorted_idxes[:-topn]
  ys = ys[sorted_idxes]
  xs = np.asarray(xs)
  xs = xs[sorted_idxes]
  return px.bar(pd.DataFrame({'y': ys, 'x': xs}), 'y', 'x', orientation='h')

def feature_importance(model, topn=0):
  model_type = type(model).__module__.lower()
  if model_type.startswith('catboost'):
    feat_importances = np.asarray(model.feature_importances_)
  elif model_type.startswith('lightgbm'):
    feat_importances = np.asarray(model.feature_importance())
  sorted_idxes = np.argsort(feat_importances)
  if topn != 0:
    if topn > 0:
      sorted_idxes = sorted_idxes[-topn:]
    else:
      sorted_idxes = sorted_idxes[:-topn]
  feat_importances = feat_importances[sorted_idxes]
  if model_type.startswith('catboost'):
    feat_names = np.asarray(model.feature_names_)
  elif model_type.startswith('lightgbm'):
    feat_names =np.asarray(model.feature_name())
  feat_names = feat_names[sorted_idxes]
  return px.bar(pd.DataFrame({'importance': feat_importances, 'feat_name': feat_names}), 'importance', 'feat_name', orientation='h') 

def feature_importance_df(model, topn=0):
  model_type = type(model).__module__.lower()
  if model_type.startswith('catboost'):
    feat_importances = np.asarray(model.feature_importances_)
  elif model_type.startswith('lightgbm'):
  ...
  if model_type.startswith('catboost'):
    ...
    feat_names =np.asarray(model.feature_name())
  feat_names = feat_names[sorted_idxes]
  return pd.DataFrame({'feat_name': feat_names, 'importanace': feat_importances})
