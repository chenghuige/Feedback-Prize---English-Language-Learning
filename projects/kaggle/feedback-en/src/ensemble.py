...
def main(_): 
  ...
  for fold in range(folds):
    ...
    # for i, x in enumerate(xs):
      x = {k: v  for k, v in x.items() if k in ['pred', 'id', 'label']}
      try:
  ...
  if mark == 'offline' and num_folds == folds:
    ...
    x['text_id'] = df_gt['text_id'].values
    df_pred = to_df(x)
  ...
  if FLAGS.kaggle_idx is not None:
    ...
    if mark == 'online':
      for i, model_dir in enumerate(model_dirs_):
        ...
        gezi.prepare_kaggle_dataset(f'{MODEL_NAME}-model', model_dir=model_dir, kaggle_idx=kaggle_idx, fold=0, method=FLAGS.kmethod, exit_last=False)
        time.sleep(10)
    else:
