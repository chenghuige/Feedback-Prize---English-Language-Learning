...
def update_positions(model, max_pos):
    ...
    model.embeddings.position_ids = torch.arange(max_pos + 2, device=model.embeddings.position_ids.device).unsqueeze(0)
...
def update_global(model, bos, mask, stride, keep_first=False):
    ...
    positions = model.embeddings.position_embeddings.weight.clone()[2:]
...
def main():
    ...
    if model_types is None:
        model_types = ["RobertaForMaskedLM"]
    ...
    # Load model
    config = LSGRobertaConfig.from_pretrained(
        args.initial_model, 
