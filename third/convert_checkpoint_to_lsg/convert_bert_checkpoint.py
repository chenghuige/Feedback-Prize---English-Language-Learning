...
def update_positions(model, max_pos):
    position_embeddings_weights = model.bert.embeddings.position_embeddings.weight.clone()
    ...
    model.bert.embeddings.position_ids = torch.arange(max_pos, device=model.bert.embeddings.position_ids.device).unsqueeze(0)
    model.bert.embeddings.position_embeddings.weight.data = new_position_embeddings_weights
    return model
...
def update_global(model, bos, mask, stride, keep_first=False):
    u = model.bert.embeddings.word_embeddings.weight.clone()
    positions = model.bert.embeddings.position_embeddings.weight.clone()
    ...
    if keep_first:
        model.bert.embeddings.global_embeddings.weight.data[1:] = positions[1:]
    else:
        model.bert.embeddings.global_embeddings.weight.data = positions
    return model
...
def update_global_randomly(model, bos, stride, keep_first=False):
    ...
    if keep_first:
        model.bert.embeddings.global_embeddings.weight.data[1:] = (w + positions)[1:]
    else:
        model.bert.embeddings.global_embeddings.weight.data = w + positions
    return model
